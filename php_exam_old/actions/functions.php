...
function User_exists_Modif($data,$strdata,$userId){
    $mysql = new PDO('mysql:host=localhost;dbname=php_exam_db;charset=utf8;','root','');
    $req_User_Exists = $mysql->prepare("SELECT $strdata FROM users WHERE $strdata= ? AND ID_User!=?");
    $req_User_Exists->execute(array($data,$userId));
    return $req_User_Exists;
}
